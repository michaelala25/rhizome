...
- **commit_proposal.py** — `CommitProposalVM`. Owns the entry list, cursor, exclusion set,
  edit-instructions buffer, and the `EDITING → DONE` lifecycle. Holds one child
  `EntryDetailsVM` that's reseeded on every cursor move. See the file docstring for the full
  state contract.

- **entry_details.py** — `EntryDetailsVM`. Per-entry buffered edit of title/content, mirroring
