"""ConversationAreaModel — the conversation half of the chat pane.

Owns everything about a single conversation: the feed (a ``ConversationGraph`` of branches), the
agent sessions bound to each leaf, commit mode, session mode, and the input / command-palette /
status-bar sub-VMs. It does NOT own panel layout or the resource viewer — those live on the
orchestrator (``ChatPaneModel``), which composes this VM and injects the shared ``ResourceManager``
used for agent bootstrap.

Escalation: actions this VM can't perform itself (open/close tabs, quit, toggle the resource
viewer, rename the enclosing tab) are emitted via ``OnNotification`` / ``OnTabRenamed``; the
``ConversationArea`` view handles the conversation-local ones and forwards the rest up to the
orchestrator view as Textual messages.
"""
...
class ConversationAreaModel(ViewModelBase):
    ...
    def __init__(
        ...
        *,
        ...
        # Per-channel feed events (so a single ConversationAreaModel mutation only repaints the touched
        ...
        self.make_callback_groups({
            ...
            self.Callbacks.OnNotification: ConversationAreaModel.NotifyAction,   # action enum value
        })
        ...
        self.state: ConversationAreaModel.State = ConversationAreaModel.State.CONVERSATION
        ...
        # session node by default and can still reach Root via ``at_scope(OptionScope.Root)``. Absent in
        ...
        # ``detach_options`` (the view's ``on_unmount``). ``Options`` satisfies the ``OptionService``
        # protocol directly, so the bare Session node is what gets registered/shadowed under the key.
        option_service = services.try_get(OptionService)
        self._options: Options | None = None
        if option_service is not None:
            self._options = Options(OptionScope.Session, parent=option_service.at_scope(OptionScope.Root))
    ...
    def append_message(
        ...
        *,
        ...
        """
        assert self.state == ConversationAreaModel.State.CONVERSATION
        target = cursor if cursor is not None else self._cursor
    ...
    def hint_higher_verbosity(self) -> None:
        ...
        present the cue.
        """
        self.emit(self.Callbacks.OnNotification, ConversationAreaModel.NotifyAction.HINT_HIGHER_VERBOSITY)
    ...
    def _on_input_submitted(self, text: str) -> None:
        ...
        """
        if self.state == ConversationAreaModel.State.COMMIT:
        ...
        if self._conversation.children(self._cursor.head):
            self.emit(self.Callbacks.OnNotification, ConversationAreaModel.NotifyAction.DESCEND_REQUIRED)
            return
    ...
    def navigate_commit_cursor_up(self) -> None:
        assert self.state == ConversationAreaModel.State.COMMIT
    ...
    def _reset_commit_state(self) -> None:
        ...
        self.state = ConversationAreaModel.State.CONVERSATION
    ...
    def _register_commands(self) -> None:
        ...
        def _options(*, global_: bool) -> None:
            ...
            if self._options is None:
                ...
                return
            # ``--global`` reaches the Root node via ``at_scope``; the default targets this
            # conversation's own Session node (the instance registered at this scope).
