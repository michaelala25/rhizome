"""Shell command — sub-VM + view used by the MVVM chat area.

Buffer entries starting with ``!`` are dispatched as shell commands by the chat area: a ``ShellCommandModel``
is appended to the feed and its ``execute()`` coroutine is scheduled on the chat area's worker. The VM owns the
...
class ShellCommandModel(ViewModelBase):
    ...
    Lifecycle:
      ...
      - caller schedules ``execute()`` on a worker (e.g. the chat area's worker scheduler) — the coroutine runs
