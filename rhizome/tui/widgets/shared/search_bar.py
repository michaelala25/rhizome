...
Layout
------
``SearchBar`` is a ``Horizontal`` container — the chrome — composing two children:
...
class _SearchField(Input):
    ...
    ``SearchBar`` so the rapid-double-tap gesture state lives in one place. Rendered
    ...
    def on_input_submitted(self, event: Input.Submitted) -> None:
        ...
        if event.input is not self:
            return
        parent = self.parent
        assert isinstance(parent, SearchBar)
...
class _ClearButton(Static):
    ...
    bubbles a ``Click`` event up to ``SearchBar.on_click``, where it's filtered on widget
...
class SearchBar(Horizontal, Generic[VM]):
    DEFAULT_CSS = """
    SearchBar {
        background: transparent;
        border: solid #3a3a3a;
