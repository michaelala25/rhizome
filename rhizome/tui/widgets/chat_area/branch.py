...
Lives in the parent node's feed (appended by ``ChatAreaModel.branch()`` at the moment of /branch).
Displays the branches reachable from that point and, when the cursor has descended through it, which
branch is currently selected. State is push-driven: the chat area walks the visible feed on every
...
the chat area VM to mutate the cursor (``descend_into`` / ``ascend`` / ``swap_sibling``).
"""
...
class BranchPoint(NavigableFeedItemViewBase[BranchPointModel]):
    ...
    # leave ``ctrl+up`` / ``ctrl+down`` free for the chat area's feed-wide navigation. Up/down are
    ...
    def __init__(self, vm: BranchPointModel, **kwargs) -> None:
