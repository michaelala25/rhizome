"""``FlashcardProposalChoices`` — vertical action menu at the bottom of the proposal widget.
...
on the parent ``FlashcardProposal`` (``ctrl+a/e/r/c``) and the in-menu cursor are intentionally
redundant — the user can either chord the binding from anywhere or arrow-navigate the menu and
press ``enter``.

Plain ``up`` / ``down`` move the menu cursor; at the first/last choice the binding is reported
as inactive via ``check_action`` so the keystroke bubbles to the parent's focus-graph bindings
and advances focus into the flashcard list or edit-instructions area.
"""
...
class FlashcardProposalChoices(ChoiceList[FlashcardProposalVM]):
    ...
    def _edit(self) -> None:
        ...
        if self._vm.edit_instructions_visible:
            try:
                self.screen.query_one("#fp-edit-instructions").focus()
            except Exception:
                pass
    ...
    def _render_choice(self, label: str, selected: bool) -> Text:
        ...
        # Greys mirror the flashcard-list keybindings hint row for visual consistency: brighter
        # ``#a0a0a0`` for the key, dimmer ``#707070`` for the descriptive text. When blurred, the
        # cursor row collapses into the rest of the list (arrow loses highlight, label drops
        # bold).
