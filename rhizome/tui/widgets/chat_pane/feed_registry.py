...
The chat pane renders an open, runtime-varying set of view-models through one feed. Rather than the
pane statically knowing every concrete type, each view declares which VM it renders by decorating
itself with ``@register_feed_view(SomeVM)``; the pane then dispatches with ``view_for(vm)``.
...
the views. That "something" is ``feed_views.py`` — import it (the pane does) before calling
