...
Owns the topic tree VM and the ordered list of pane VMs (one per tab), plus the active-tab index. The job
...
orchestrator just arranges that:
  ...
  * when the user changes the tree selection, the active pane gets the expanded filter immediately;
  * inactive panes are caught up lazily on tab switch — their data and last-applied filter persist until
    then;
...
pane. There is no "broadcast filter" callback group — that would be one VM emitting on behalf of another.

Lazy propagation
----------------
Filter changes fan out only to the active pane. Inactive panes hold whatever data they last rendered, plus
the filter that produced it. On tab switch the new active pane gets a ``set_filter`` with the current
orchestrator filter; because ``BrowserPaneViewModel.set_filter`` is idempotent on unchanged filters,
switching to a pane that already matches the current filter is an instant no-op, while switching to a pane
...
``expanded_filter_ids`` followed by a direct ``set_filter`` on the active pane — no background task, no
cancellation dance.
"""
...
def _default_panes(session_factory: Any) -> list[BrowserPaneViewModel]:
    """The production pane set, constructed at ctor time when the caller doesn't override. Kept as a free
...
class BrowserViewModel(ViewModelBase):
    ...
    The pane lineup is fixed at construction. Pass ``panes=...`` to override the default (e.g. in tests);
    pass ``panes=None`` to get the production set from ``_default_panes``. Call ``await start()`` once after
    mounting to load the tree roots and seed the active pane with the empty filter.
    """
    ...
    def __init__(
        ...
        *,
        ...
        # ``_current_filter`` is the orchestrator's view of the live filter — the active pane is brought in
        # sync with this immediately; inactive panes catch up via ``switch_pane``. ``frozenset`` ↔ ``None``
        # echoes the pane-side filter semantics (None = no filter at all).
    ...
    async def start(self) -> None:
        ...
        if self._started:
            return
        self._started = True
        # Seed only the active pane. Inactive panes stay empty until the user switches to them (see
        # ``switch_pane``).
    ...
    def _add_pane(self, pane: BrowserPaneViewModel) -> None:
        """Append a pane to the tab list. Private — the pane lineup is fixed at construction. Called from
        ``__init__`` only."""
        self._panes.append(pane)
        self.emit(self.dirty)

    def switch_pane(self, index: int) -> None:
        """Activate the pane at ``index``. No-op if already active or out of range — out-of-range is a
        programmer error, but we'd rather log and ignore than crash a UI handler.

        Catches the newly-active pane up to ``_current_filter`` via ``set_filter``; the call is a no-op if
        the pane already holds data for that filter (instant switch), or triggers a refetch if not.
        """
        ...
        # Only sync the new active pane with the current filter after we've started — before start() the
        # tree hasn't loaded, ``_current_filter`` is still ``None`` by default, and nobody should be
        # switching panes anyway.
        if self._started:
    ...
    def _on_selection_changed(self) -> None:
        ...
        here: read the now-expanded filter set and hand it straight to the active pane. Inactive panes
        catch up lazily on ``switch_pane`` via the idempotent ``set_filter``.
        """
