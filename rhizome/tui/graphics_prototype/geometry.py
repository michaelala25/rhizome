...
A bitmap is shown by fitting it into the widget's content box (a rectangle measured in terminal
cells, here already converted to pixels) keeping its aspect ratio, centered/aligned on a background
canvas. ``Placement`` captures that fit once; the backend applies it to *draw* the page and a hover
hit-test ``footprint``-s a mouse cell back through it to *find* the box under the cursor. Computing
the transform in exactly one place is what keeps the rendered page and the boxes hit-tested against
