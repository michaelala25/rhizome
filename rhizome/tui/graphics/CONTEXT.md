...
- **`backend.py`** — `GraphicsBackend`, the protocol-neutral contract: `prepare` (main-thread
  ...
  reject (no half-cell fallback). Detection queries the terminal, so call it **at startup**.
- **`sixel.py`** — `SixelBackend`, the implemented protocol (libsixel `img2sixel`). Encodes the
  ...
  z-index). See the module docstring for the protocol's quirks.
...
- **`widget.py`** — `GraphicsImage`, the Textual widget that drives a backend: it owns the future-cache
  ...
  payloads are opaque; an owner interprets them. `prefetch(bitmap)` warms a neighbour frame.
...
- **Single point of `textual_image` coupling.** We reuse only its terminal queries — `get_cell_size`
  and `renderable.{sixel,tgp}.query_terminal_support` — and own encoding + placement ourselves. Pin
  the version; that surface is small and terminal-protocol-stable.
