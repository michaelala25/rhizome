...
- **topics.py** — `create_topic` (with optional `parent_id`), `get_topic`, `list_root_topics`, `list_children`, `find_parent_topic_ids` (batch "does this topic have children?" check used by the browser topic tree to populate expand-affordances without N+1 queries), `get_subtree` (recursive CTE, depth-limited to 10, hydrates Topic rows), `expand_subtrees` (recursive CTE over multiple roots, returns just `set[int]` of IDs — no ORM hydration, used by the browser's multi-select topic filter), `update_topic`, `delete_topic`. Topics form a tree via adjacency list (`parent_id` self-FK). Deleting a topic cascades to its entries but not to child topics (FK constraint prevents deleting a topic with children).

- **entries.py** — `create_entry`, `get_entry`, `list_entries`, `list_entries_paginated`, `count_entries_filtered`, `update_entry`, `delete_entry`, `search_entries`. Entries have an `entry_type` (default "fact"), `difficulty` (nullable int), and `speed_testable` (bool, default false). `search_entries` does case-insensitive LIKE on title+content, with optional `topic_id` scoping. `list_entries_paginated` and `count_entries_filtered` back the browser pane: both take `topic_ids` (`None` = no filter, empty iterable = "no rows"), `search` (LIKE on title+content), `entry_types` (`None`/iterable of `EntryType`, same empty-iterable convention), and `has_flashcards` (`bool | None` — `True` filters to entries with at least one linked flashcard via an EXISTS subquery on `FlashcardEntry`, `False` to entries with none via NOT EXISTS, `None` skips the predicate). They share a single `_apply_entry_filters` helper so the count matches the window exactly. Sorting is restricted to `EntrySortKey` (`id`/`title`/`created_at`/`updated_at`) with an `id` tiebreaker for stable pagination.
